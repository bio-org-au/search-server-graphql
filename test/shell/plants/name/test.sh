...
detail_fields='{ id, simple_name, full_name, full_name_html, family_name, name_status_name, name_usages { instance_id, standalone, instance_type_name, reference_details {id, citation, citation_html, page, page_qualifier, year, full_citation_with_page}, primary_instance, misapplication, misapplication_details {direction, misapplied_to_full_name, misapplied_to_name_id, misapplied_in_references {id, citation, page, page_qualifier, display_entry}, misapplication_type_label }, synonyms { id, full_name, instance_type, label, page, name_status_name, }, accepted_tree_details { is_accepted, is_excluded, comment { key, value }, distribution { key, value } }, non_current_accepted_tree_details { comment { key, value }, distribution { key, value } }, notes { key, value } } }'
...
diff -q ${test_name}.expected $temp_dir/${test_name}.log
sed -e 's/{/{/g' -e 's/,/,/g' -e 's/\\u003c/\</g' -e 's/\\u003e/>/g' ${test_name}.expected >$temp_dir/t${test_number}.expected
sed -e 's/{/{/g' -e 's/,/,/g' -e 's/\\u003c/\</g' -e 's/\\u003e/>/g' $temp_dir/${test_name}.log >$temp_dir/t${test_number}.log
diff  $temp_dir/t${test_number}.expected $temp_dir/t${test_number}.log
...
curl -s -d "query={ name(id: 91755) ${detail_fields} }" -X POST http://localhost:2004/v1 -o $temp_dir/${test_name}.log
sed -i '' -e '$a\' $temp_dir/${test_name}.log
diff -q ${test_name}.expected $temp_dir/${test_name}.log
sed -e 's/{/{/g' -e 's/,/,/g' -e 's/\\u003c/\</g' -e 's/\\u003e/>/g' ${test_name}.expected >$temp_dir/t${test_number}.expected
sed -e 's/{/{/g' -e 's/,/,/g' -e 's/\\u003c/\</g' -e 's/\\u003e/>/g' $temp_dir/${test_name}.log >$temp_dir/t${test_number}.log
...
curl -s -d "query={ name(id: 87221755) ${detail_fields} }" -X POST http://localhost:2004/v1 -o $temp_dir/${test_name}.log
diff -q ${test_name}.expected $temp_dir/${test_name}.log
sed -e 's/{/{/g' -e 's/,/,/g' -e 's/\\u003c/\</g' -e 's/\\u003e/>/g' ${test_name}.expected >$temp_dir/t${test_number}.expected
sed -e 's/{/{/g' -e 's/,/,/g' -e 's/\\u003c/\</g' -e 's/\\u003e/>/g' $temp_dir/${test_name}.log >$temp_dir/t${test_number}.log
diff  $temp_dir/t${test_number}.expected $temp_dir/t${test_number}.log
