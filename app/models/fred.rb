class Fred
  ...
  def scientific_search_sql
    @sql = Name.where(['lower(simple_name) like lower(?) or lower(full_name) like lower(?)', preprocessed_search_term, preprocessed_search_term])
        .joins(:name_type)
  ...
  def preprocessed_search_term
    ...
    throw 'no search term' if @args['search_term'].blank?
