class NameSearch
  ...
  def self.scientific_search
    ...
    Name.where(['lower(simple_name) like lower(?) or lower(full_name) like lower(?)',self.preprocessed_search_term,self.preprocessed_search_term])
        .joins(:name_type)
  ...
  def self.preprocessed_search_term
    if @args["search_term"].blank?
      ...
      @args["search_term"].sub(/$/,'%').gsub(/\*/,'%')
    else
      @args["search_term"].gsub(/\*/,'%')
    end
  end
end
